...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/vm-problem1.png)

- 程序如何分配内存（对于碎片内存）

就像下面这张图，首先内存加载程序p1和p2，开始3G空间被占用，剩余1G空间剩余，这时候p1退出，p3想使用内存，虽然还有2G剩余空间，但空间是碎片化的，这也是一个问题。这被称作**Memory fragmentation**`内存碎片化`。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/vm-problem2.png)

- 如何保证进程安全（进程之间地址不相互访问）

就像下面这张图，两个程序同时访问`1024`这个地址的空间，内存中的信息会不会相互覆盖？

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/vm-problem3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/whatis-vm1.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/not-enough-memory.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/memory-hole.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/program-secure.png)

这样一来，因为每个程序都有自己的地址空间，那么他们不可能访问其他程序的数据，这非常的安全和可靠！

这也是有缺点的，当两个程序合作的时候，如何共享内存呢？**只需要将两个程序各自的地址空间拿出一部分映射到相同的RAM内存**。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/memory-share.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/vm-work1.png)

- 计算机通过物理地址读取RAM并将数据返还给程序。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/vm-work2.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/page-table1.png)

那么如何改造呢？以上情况的映射是1个字对应一个PTE页表入口，我们称这种情况为`细粒度映射`，那我们可不可以使用`粗粒度映射`呢？就像这个图一样：

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/page-table2.png)

这个图中，我们将每4Kb大小的地址空间做一个PTE，我们称之为`4KB Pages`，因为每个RAM按字节计量，或者你可以看为1K个字大小的页。这样我们就减少了映射也就是PTE的个数。这4KB大小的页映射的内部是连续的，如下图：

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/page-table3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/addr-trans1.png)

那我们4KB大小的页内部没有做映射，这一部分如何映射呢？解决办法就是使用`偏移量offset`，如下图

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/addr-trans2.png)

我们将一个虚拟地址转换为物理地址，虚拟地址的pagenumber部分通过页表转换为实际物理地址的pagenumber部分，虚拟地址的offset部分直接作为物理地址的offset部分。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/addr-trans3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/addr-trans4.png)

当然，如果查页表的时候可能会产生`page fault`，即所需要的内容没有在RAM中，如图中的**0x00000**，这就需要发生中断，加载内容到RAM，然后修改页表，返回到内存上次执行的位置，这时候就会找到所对应的物理页编号了。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/addr-trans5.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/addr-trans6.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-protection1.png)

这说明每一个程序都有自己的32位虚拟地址空间，下面是Linux虚拟内存空间的使用情况。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-protection2.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-protection3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-protection4.png)
### 如何实现虚拟内存
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-fast1.png)

最初，通过访问RAM中的页表来将虚拟地址转换为物理地址，这是非常慢的操作。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-fast2.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/mem-fast3.png)

- 让硬件自动填充TLB（因为不会经过操作系统的处理）。这被称为`Hardware Page Table Walk`，由MMU来完成。

下面是如何通过TLB将虚拟地址转化为物理地址的例子。

![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-example1.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-example2.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-example3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-example4.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/multi-level-page-table1.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/multi-level-page-table2.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/multi-level-page-table3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-cache1.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-cache3.png)
...
![](https://blog-resource-1259125863.cos.ap-beijing.myqcloud.com/images/virtual-memory/tlb-cache4.png)
